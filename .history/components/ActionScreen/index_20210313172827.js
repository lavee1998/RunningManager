...
const ActionScreen = ({ navigation }) => {
  ...
  const updatePosition = (currLocation) => {
    ...
    let lastLocation = runCoordinates.slice(-1)[0]
    if(lastLocation) {
      let start = {
      ...
      let end = {
        latitude: runCoordinates.slice(-1)[0].coords.latitude, //last coordinate
        longitude: lastLocation.coords.longitude
      }
      const currDistance = haversine(start, end, {unit: 'meter'})
